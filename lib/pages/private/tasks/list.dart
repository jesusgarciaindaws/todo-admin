...
class TasksPage extends anxeb.PageWidget<Application, PageMeta> {
  ...
  @override
  String title({anxeb.GoRouterState state}) => 'Gestión de tareas';
}
...
class _TasksState extends anxeb.PageView<TasksPage, Application, PageMeta> {
  ...
  void setup() async {
    ...
    if (info?.state?.queryParams != null &&
        info?.state?.queryParams['refresh'] == 'true') {
      _refresh();
    }
  }
  ...
  List<UserModel> get users => _users != null
      ? _users.where(($item) => $item.filter(lookup: _lookup)).toList()
      : _users;
